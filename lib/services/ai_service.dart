...
class AIService {
  ...
  Future<String> getChatResponse({
    ...
    final cacheParams = {
      ...
      // We don't include 'places' in cache key to avoid massive keys. 
      // Assumption: Place list for a city creates a stable context enough context for the same question.
    };
    
    final cacheKey = AICacheService.instance.generateCacheKey(cacheParams);
    final cachedResponse = await AICacheService.instance.getCachedResponse(cacheKey);
    
    if (cachedResponse != null) {
      print("🚀 AI Chat Cache HIT: $cityName - $question");
    ...
    try {
      ...
      ÖNEMLİ: Eğer bir yer öneriyorsan, adını MUTLAKA şu formatta yaz: [Yer Adı](search:Yer Adı)
      Örnek: "Prag'da kahve için [Ema Espresso Bar](search:Ema Espresso Bar) harikadır."
      Parantez içindeki "search:" kısmını ASLA çevirme, olduğu gibi kalsın. Yer adını da değiştirme.
      ''';

      final content = [Content.text(prompt)];
      final response = await model.generateContent(content);
      final responseText = response.text ?? (isEnglish ? "Sorry, couldn't generate a response." : "Üzgünüm, yanıt oluşturulamadı.");

      // 2. Cache Save (if successful)
      if (response.text != null) {
        // 🔥 Fire-and-forget: Don't await, let it save in background
        AICacheService.instance.cacheResponse(
          key: cacheKey,
  ...
  static Future<String> getPersonalizedChatResponse({
    ...
    required String userName, // Not used in prompt anymore for caching, but kept in signature for compatibility
    ...
    // 1. Cache Check
    final cacheParams = {
      'city': cityModel.city.toLowerCase().trim(),
      'style': travelStyle.toLowerCase().trim(),
    ...
    if (cachedResponse != null) {
      print("🚀 AI Suggestion Cache HIT: ${cityModel.city}");
      return cachedResponse;
    }
    ...
    try {
      ...
      // Rastgelelik için seed oluştur (Cache miss durumunda yeni/farklı bir şey üretmesi için)
      ...
      // Generic user name for caching friendly prompt
      final genericUserName = isEnglish ? "Traveler" : "Gezgin"; 

      final prompt = '''
      $languageInstruction
      
      You are a local guide for ${cityModel.city}.
      User: $genericUserName. Style: $travelStyle. Interests: ${interests.join(", ")}. Budget: $budgetLevel. Trip: $tripDays days.
      ...
      Start with a friendly greeting paragraph (address the user as $genericUserName).
      ...
      if (response.text != null) {
        // 2. Cache Save - 🔥 Fire-and-forget for speed
        AICacheService.instance.cacheResponse(
          key: cacheKey,
...
- [Vigeland Parkı](search:Vigeland Parkı) - Tek bir sanatçı tarafından yapılan dünyanın en büyük heykel parkı. 200'den fazla bronz ve granit heykel. Tuhaf, harika ve ücretsiz! Piknik için mükemmel.''',
  ...
  static Future<String> getCityBlogContent(String city, bool isEnglish) async {
    ...
    try {
      ...
      if (cityModel.guide != null) {
        final guide = cityModel.guide!;
        StringBuffer sb = StringBuffer();

        // Intro
        sb.writeln(isEnglish ? guide.introEn : guide.intro);
        sb.writeln();

        // Recommendations
        if (isEnglish) {
          if (guide.recommendationsEn.isNotEmpty) sb.writeln(guide.recommendationsEn);
        } else {
          if (guide.recommendations.isNotEmpty) sb.writeln(guide.recommendations);
        }
        sb.writeln();

        // Transport
        if (isEnglish) {
          if (guide.transportEn.isNotEmpty) sb.writeln(guide.transportEn);
        } else {
          if (guide.transport.isNotEmpty) sb.writeln(guide.transport);
        }
        sb.writeln();

        // Tips
        if (isEnglish) {
          if (guide.tipsEn.isNotEmpty) sb.writeln(guide.tipsEn);
        } else {
          if (guide.tips.isNotEmpty) sb.writeln(guide.tips);
        }
        sb.writeln();
        
        // Hidden Gems (Eğer varsa)
        if (isEnglish) {
           if (guide.hiddenGemsEn.isNotEmpty) sb.writeln(guide.hiddenGemsEn);
        } else {
           if (guide.hiddenGems.isNotEmpty) sb.writeln(guide.hiddenGems);
        }

        if (sb.isNotEmpty) return sb.toString();
      }
    } catch (e) {
      print("⚠️ City guide loading error from JSON for $city: $e");
    }

    // 2. Fallback: CityBlogContent sınıfından içeriği al (Hardcoded)
